module.exports = function (io, chat, recentMessages, session) {
    ...
    io.on('connection', function (socket) {
        ...
        socket.on('user-connect', function (client) {
            ...
            session.room = client.room;
            session.nick = client.nick;
            session.socket = client.socketID;
            chat.sendMessage('recent-msgs', 'server', socket.id, recentMessages[client.room]);
            chat.sendMessage('msg-to-room', 'admin', client.room, client.nick +' has joined ' + client.room + '.');
