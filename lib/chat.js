module.exports = function (io, roomList) {
    ...
    module.checkIfNameIsTaken = function (room, nick) {
        nick = nick.toLowerCase();
        if (roomList[room]) {
            if (roomList[room].users.hasOwnProperty(nick)) {
                return true;
            }
        }
        return false;
    ...
    module.changeName = function (room, oldname, newname) {
        roomList[room].users[newname.toLowerCase()] = roomList[room].users[oldname.toLowerCase()];
        delete roomList[room].users[oldname.toLowerCase()];
